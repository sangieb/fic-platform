...
public class ReporteController {
    ...
    public ResponseEntity<String> descargarPDF() {
        ...
        String colFondo    = messageSource.getMessage(
            "tabla.columna.fondo", null, locale);
        String colGestora  = messageSource.getMessage(
            "tabla.columna.gestora", null, locale);
        String colDiaria   = messageSource.getMessage(
            "tabla.columna.diaria", null, locale);
        String colMensual  = messageSource.getMessage(
            "tabla.columna.mensual", null, locale);
        String colAnual    = messageSource.getMessage(
            "tabla.columna.anual", null, locale);
        String colPeriodo  = messageSource.getMessage(
            "tabla.columna.periodo", null, locale);

        // Construye las filas HTML
        StringBuilder filas = new StringBuilder();
        for (FicData f : fondos) {
            String colorDiaria  = colorRent(f.getRentabilidadDiaria());
            String colorMensual = colorRent(f.getRentabilidadMensual());
            String colorAnual   = colorRent(f.getRentabilidad());

            filas.append("<tr>")
                .append("<td>").append(safe(f.getNombreFondo())).append("</td>")
                .append("<td>").append(safe(f.getSociedadGestora())).append("</td>")
                .append("<td style='color:").append(colorDiaria).append(";font-weight:600'>")
                    .append(formatRent(f.getRentabilidadDiaria())).append("</td>")
                .append("<td style='color:").append(colorMensual).append(";font-weight:600'>")
                    .append(formatRent(f.getRentabilidadMensual())).append("</td>")
                .append("<td style='color:").append(colorAnual).append(";font-weight:600'>")
                    .append(formatRent(f.getRentabilidad())).append("</td>")
                .append("<td>").append(safe(f.getPeriodo(), 10)).append("</td>")
                .append("</tr>");
        }

        // HTML completo con estilos de impresión
        String html = """
            <!DOCTYPE html>
            <html>
            <head>
            ...
            """.formatted(
                titulo, titulo,
                generado, fecha, fondos.size(),
                colFondo, colGestora,
                colDiaria, colMensual, colAnual, colPeriodo,
                filas.toString()
            );
        ...
        reporteRepo.save(r);
        auditService.registrar("Reporte", "PDF", "CREATE",
            "sistema", "Generacion reporte PDF");
    ...
    public ResponseEntity<byte[]> descargarExcel() {
        ...
        String titulo   = messageSource.getMessage(
            "reporte.excel.titulo", null, locale);
        String generado = messageSource.getMessage(
            "reporte.generado", null, locale);
        String fecha    = LocalDateTime.now()
            .format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm"));
        ...
        String colGestora  = messageSource.getMessage(
            "tabla.columna.gestora", null, locale);
        String colTipo     = messageSource.getMessage(
            "tabla.columna.tipo", null, locale);
        String colDiaria   = messageSource.getMessage(
            "tabla.columna.diaria", null, locale);
        String colMensual  = messageSource.getMessage(
            "tabla.columna.mensual", null, locale);
        String colAnual    = messageSource.getMessage(
            "tabla.columna.anual", null, locale);
        String colInv      = messageSource.getMessage(
            "tabla.columna.inversionistas", null, locale);
        String colPeriodo  = messageSource.getMessage(
            "tabla.columna.periodo", null, locale);

        StringBuilder csv = new StringBuilder('\uFEFF'); // BOM para Excel
        ...
        for (FicData f : fondos) {
            csv.append(csvCelda(f.getNombreFondo())).append(",")
               ...
               .append(f.getRentabilidadDiaria() != null
                   ? f.getRentabilidadDiaria() : "").append(",")
               .append(f.getRentabilidadMensual() != null
                   ? f.getRentabilidadMensual() : "").append(",")
               .append(f.getRentabilidad() != null
                   ? f.getRentabilidad() : "").append(",")
               .append(f.getNumInversionistas() != null
                   ? f.getNumInversionistas() : "").append(",")
               .append(csvCelda(f.getPeriodo() != null
                   ? f.getPeriodo().substring(0,
                       Math.min(10, f.getPeriodo().length())) : ""))
               .append("\n");
        }
        ...
        r.setGeneratedAt(LocalDateTime.now());
        reporteRepo.save(r);
        auditService.registrar("Reporte", "EXCEL", "CREATE",
            "sistema", "Generacion reporte Excel");

        return ResponseEntity.ok()
            .header(HttpHeaders.CONTENT_DISPOSITION,
                "attachment; filename=reporte-fic.csv")
